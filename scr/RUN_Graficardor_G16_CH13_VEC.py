...
Calibra a Tb [Rad]->[C](lee los coeficiente de calibracion del archivo auxiliar)
...
def fecha(t0): #con el tiempo en segundos devuelve strings de fecha para las figuras (t) y nombres de los archivos (N)

    mes=['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic']   
    fechaT=('%i-%s-%02d %02d:%02d:%02d UTC' %(t.gmtime(t0)[0],mes[t.gmtime(t0)[1]]
    ,t.gmtime(t0)[1],t.gmtime(t0)[3],t.gmtime(t0)[4],t.gmtime(t0)[5]))            
...
def ploteador(nombre):  
    ...
    #sourcePrj.ImportFromProj4('+proj='+proj+' +h='+h+' +a='+a+' +b='+b+' +f='+str(f)#no es valida proj+'geostationary'
    ...
    #if icanal>=7:#DN ->C
    #    Temp=(fk2 / (np.log((fk1 / array) + 1)) - bc1 ) / bc2-273.15#mW m-2 sr-1 mum-1 ->C
    #else:
    ...
    t_ini=t_0+ float(calendar.timegm(t.strptime(t_start, '%Y-%m-%d %H:%M:%S')))
    ...
    bmap.readshapefile(dires+'008_limites_provinciales_LL','008_limites_provinciales_LL',linewidth=0.3,color='w')
    bmap.readshapefile(dires+'DEPARTAMENTOS_linea','DEPARTAMENTOS_linea',linewidth=0.1,color='gray')#bmap.readshapefile(dires3+'Límites_internacionales','Límites_internacionales',linewidth=0.3,color='w')
    ...
    if icanal>=7:
        img_plot=bmap.imshow(Temp, origin='upper', cmap=cpt_convert, vmin=-90, vmax=50)
    else:
        img_plot=bmap.imshow(Temp, origin='upper', cmap='Greys', vmin=0.001, vmax=0.1)#'Greys'
    
    
    # Add a black rectangle in the bottom to insert the image description
    lon_difference = (extent[2] - extent[0]) # Max Lon - Min Lon
    currentAxis = plt.gca()
    currentAxis.add_patch(Rectangle((extent[0], extent[1]), lon_difference, (lon_difference) * 0.040, alpha=1, zorder=3, facecolor='black'))
    ...
    plt.text(extent[0], extent[1] + lat_difference * 0.005,Title,horizontalalignment='left', color = 'white', size=5)
    plt.text(extent[2], extent[1] + lat_difference * 0.005,Institution, horizontalalignment='right', color = 'white', size=5)
    ...
    #driver.CreateCopy(dire+'Channel_'+ canal+'_'+Region+fechaN+'_WGS84.tif', grid, 0)
    
    #grabar a png
    
    
    ##Volver este
    #plt.savefig(dire_out+'Channel_'+ canal+'_'+Region+'_'+date+'_'+time+'_WGS84.png', dpi=300,bbox_inches='tight', pad_inches=0)
    ...
    #grid.GetRasterBand(1).WriteArray(array)
    #driver = gdal.GetDriverByName('GTiff')
    #driver.CreateCopy(dire_out+'Channel_'+ canal+'_'+Region+'_'+date+'_'+time+'_WGS84.tif', grid, 0)

...
#resultado=dia+"/nc/minimo_diario/minimo_d_"+dia+".nc"


#folder2 = '/media/andres/Elements/GOES16/BTCH13/IMAGENES/'+dia+'/' ### a las imagens origen

folder = '/home/alighezzolo/BTCH13/OUTPUTS/NC/'+dia+'/minimo_diario/' ###acceso al .nc
...
#dire_out='/media/andres/Elements/GOES16/Estadistica/recortes/'
...
archiM='/home/alighezzolo/BTCH13/OUTPUTS/NC/'+dia+'/minimo_diario/minimo_d_'+dia+'.txt'
arch = open (archiM,'r')
...
connectionInfo_0 = 'HDF5:\"' + folder+'minimo_d_'+dia+'.nc' + '\"://'+VAR
...
#array_01 = array_01 #* scale + offset 
...
bmap.readshapefile(dires+'008_limites_provinciales_LL','008_limites_provinciales_LL',linewidth=0.5,color='w')
bmap.readshapefile(dires+'DEPARTAMENTOS_linea','DEPARTAMENTOS_linea',linewidth=0.2,color='w')#bmap.readshapefile(dires3+'Límites_internacionales','Límites_internacionales',linewidth=0.3,color='w')


bmap.drawparallels(np.arange(-90.0, 90.0, 5.), linewidth=0.3, dashes=[4, 4], 
                       color='white', labels=[True,True,False,False], fmt='%g', labelstyle="+/-", xoffset=0.10, yoffset=-1.00, size=5)
bmap.drawmeridians(np.arange(0.0, 360.0, 5.), linewidth=0.3, dashes=[4, 4], 
...
img_plot=bmap.imshow(array_01-273.15, origin='upper', cmap='Spectral', vmin=-100, vmax=50)
...
Title = " GOES-16 ABI Canal"

plt.savefig(folder+'minimo_d_'+dia+'_r.png', dpi=300,bbox_inches='tight', pad_inches=0)
...
driver.CreateCopy(folder+'minimo_d_'+dia+'.tif', grid_minimo, 0)
